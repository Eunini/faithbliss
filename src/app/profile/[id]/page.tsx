...
import { 
  ArrowLeft, Heart, X, MessageCircle, MapPin, Church, BookOpen, Music,
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setProfile(userData); // Use userData directly
      } catch (error) {
                ...
                <div className="flex items-center space-x-2">
                  {profile.lookingFor && profile.lookingFor.length > 0 && (
                    profile.lookingFor.map((goal, index) => (
                      <div key={index} className="bg-blue-500/20 text-blue-300 px-3 py-1 rounded-full border border-blue-500/30 text-sm">
                        {goal}
                      </div>
                    ))
                  )}
              ...
              <div className="bg-gradient-to-r from-purple-600/20 to-pink-600/20 rounded-2xl p-4 border border-purple-500/30">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-white italic">&quot;{profile.favoriteVerse}&quot;</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profile.faithJourney && (
                <div className="bg-gray-800/50 rounded-xl p-4">
                  <h4 className="font-semibold text-blue-300 mb-2">Faith Journey</h4>
                  <p className="text-gray-300">{profile.faithJourney}</p>
                </div>
              )}
              {profile.sundayActivity && (
                <div className="bg-gray-800/50 rounded-xl p-4">
                  <h4 className="font-semibold text-blue-300 mb-2">Sunday Activity</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profile.fieldOfStudy && (
                <div className="bg-gray-800/50 rounded-xl p-4">
                  ...
                  <p className="text-gray-400">{profile.fieldOfStudy}</p>
                </div>
              )}
              {profile.profession && (
                <div className="bg-gray-800/50 rounded-xl p-4">
