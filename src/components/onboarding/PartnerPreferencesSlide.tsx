  ...
  const handleMultiSelect = (
    ...
    setOnboardingData(prev => {
      ...
      if (name === 'preferredDenomination') {
        const currentJsonString = prev[name] as string | null | undefined;
        const currentList = currentJsonString ? JSON.parse(currentJsonString) as string[] : [];
        const newList = currentList.includes(value)
          ? currentList.filter((item: string) => item !== value)
          : [...currentList, value];
        return { ...prev, [name]: JSON.stringify(newList) };
      } else {
        ...
        <div className="flex flex-wrap gap-2">
          {denominationOptions.map(option => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors ${
                onboardingData.preferredDenomination && JSON.parse(onboardingData.preferredDenomination).includes(option)
                  ? 'bg-pink-600 text-white'
                  : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
